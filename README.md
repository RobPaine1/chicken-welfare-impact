...
The weight constants and their sources are in the page's script. The pain
data is in `docs/wfp.js`, generated by `scripts/fetch_paintrack.py` from the
Welfare Footprint Project's Pain-Track for broilers (per-harm hours by
intensity for the average bird, with standard deviations, for the Conventional
